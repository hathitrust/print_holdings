...
def get_infiles (log, conn)
  ...
  member_ids = get_memberids(log, conn);
...
def show_counts (member_id, log, conn)
  sql = %W<
    SELECT 
        COUNT(1) AS c, 
        item_type 
...
def process_infiles (infiles, log, conn)
  delete_sql = %W[
  ...
  infiles.each do |infile|
    ...
    if m != nil then
      ...
      show_counts(member_id, log, conn);
...
if $0 == __FILE__ then
  begin
    ...
    db   = Hathidb::Db.new();
    conn = db.get_conn();
    infiles = get_infiles(log, conn);
    process_infiles(infiles, log, conn);
