...
db    = Hathidb::Db.new();
conn  = db.get_conn();
hdout = Hathidata::Data.new("costreport/costreport_$ymd_deets.tsv").open('w');
...
if !monograph_members.include?('ucm') then
  # UCM are only sharing PD costs.
  total_members += 1;
end
...
per_member = pd_sum / total_members;
...
hdout.file.puts "Total public domain costs\t$#{pd_sum}";
hdout.file.puts "Public domain cost per member\t$#{per_member}";
hdout.file.puts "Remaining total cost\t$#{total_operating_costs - pd_sum}";
...
hdout.file.puts "Remaining total cost\t$#{total_operating_costs - pd_sum - spm_total}";
...
hdout.file.puts "Remaining total cost\t$#{total_operating_costs - pd_sum - spm_total - mpm_total}";
...
no_serial_sum = 0.0;
no_serials_list.each do |m|
  mem_percent         = percents[m];
  mem_cost            = (mem_percent * ser1_total);
  cost_struct[m][2] = mem_cost;
  cost_struct[m][3] = mem_cost;
...
if calc_full_serial
  ...
  serial_members.each do |m|
    result = CostCalc.calc_ic_serial_cost_for_member_by_count2(m, ave_cost_per_vol, conn);
    if not result
      hdout.file.puts "No data for #{m}";
      next;
    end
    xresult     = result * xfactor;
    cost_f      = "%.2f" % result.to_f;
    ...
    hdout.file.puts "\t#{m}\t#{cost_f}";
    cost_struct[m][2] = result.to_f;
  end
  total_ser2 = "%.2f" % ser2_total;
  hdout.file.puts "Total IC ser cost (full)\t$#{total_ser2}";
  hdout.file.puts "Remaining total cost (full) $#{total_operating_costs - pd_sum - spm_total - mpm_total - ser2_total}";
end
...
serial_members.each do |m|
  result = CostCalc.calc_ic_serial_cost_for_member_by_count2(m, new_ave_cost_per_vol_serial, conn);
  if not result
    hdout.file.puts "No data for #{m}";
    next;
  end
  ...
  hdout.file.puts "\t#{m}\t#{cost_f}";
  cost_struct[m][3] = result.to_f;
end

total_ser3 = "%.2f" % ser3_total;
hdout.file.puts "Total IC ser cost (reduced)\t$#{total_ser3}";
hdout.file.puts "Remaining total cost (reduced)\t$#{total_operating_costs - pd_sum - spm_total - mpm_total - ser3_total}";

pd_extra = 0;

# Redistrubute the ht cost as pd cost between the other members:
if cost_struct.has_key?('ht') then
  ht                     = cost_struct.delete('ht');
  remaining_member_count = cost_struct.keys.size;
  remaining_member_count += 1; # for ucm, not included in the hash.
  ht_subtotal            = ht.inject(:+);
  pd_extra              += ht_subtotal / remaining_member_count.to_f;
  hdout.file.puts "subtotal of the so-called ht member\t#{ht_subtotal}";
  hdout.file.puts "1/#{remaining_member_count}th of that to each member as PD cost\t#{pd_extra}";
end
