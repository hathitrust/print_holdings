...
get_unique_commitments = conn.prepare(%w{
    SELECT COUNT(*) AS c FROM (
        SELECT t1.resolved_oclc, COUNT(t2.resolved_oclc) AS h 
        FROM shared_print_commitments AS t1 
        JOIN shared_print_commitments AS t2 ON (t1.resolved_oclc = t2.resolved_oclc) 
        WHERE t1.member_id = ? 
        GROUP BY t1.resolved_oclc HAVING h = 1
    ) AS x
}.join(' '));

hdout = Hathidata::Data.new("reports/shared_print/overview_counts_$ymd.tsv").open('w');
hdout.file.puts(
  %w[member_id holdings_count pool_count commitment_count percent_pool_committed 
     unique_in_pool unique_in_hathi unique_commitments].join("\t")
);
...
Hathidata.read("shared_print_members.tsv") do |member_id|
  ...
  get_unique_commitments.query(member_id) do |row|
    unique_commitments = row[:c].to_i;
  end
  ...
  total_unique_commitments    += unique_commitments;
  ...
  if pool_count > 0 && commitment_count > 0 then
    ...
    percent_pool_committed = "#{percent_pool_committed}%";
  end

  hdout.file.puts(
    [member_id, holdings_count, pool_count, commitment_count, percent_pool_committed,
     unique_in_pool_count, unique_in_hathi_count, unique_commitments].join("\t")
  );
end
...
hdout.file.puts(
  ["TOTAL", total_holdings_count, total_pool_count, total_commitment_count, total_percent_pool_committed,
   total_unique_in_pool_count, total_unique_in_hathi_count, total_unique_commitments
  ].join("\t"));
hdout.close();
